...
%     u_min=0;
    ...
    D=300; N=100; Nu=30;   %najlepsze parametry eksperymentalne
    psi=[1 1];
    lambda=[1 1];
...
figure
for i=1:2
    subplot(2,1,i); stairs(Y{i});title(sprintf('Wartoœæ wyjœcia Y%d(k)',i));xlabel('k');ylabel('wartoœæ sygna³u');%ylim([-5,5]);
...
for i=1:2
    subplot(2,1,i); stairs(U{i}); hold on;title(sprintf('Wartoœæ wejœcia U%d(k)',i));xlabel('k');ylabel('wartoœæ sygna³u');%ylim([-5,5]);
end
hold off